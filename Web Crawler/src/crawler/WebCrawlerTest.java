...
public class WebCrawlerTest {
	...
	public static void compare(final Page expected, final Page actual, final Map<Page, Page> pages,
			final Map<Image, Image> images, final Downloader downloader) {
	...
	private static void compare(final Image expected, final Image actual, final Map<Image, Image> images,
		...
		if (other == null) {
			if (expected.getUrl() != null) {
				...
				if (expected.getFile() != null) {
					try {
						Asserts.assertEquals("Hash of " + expected.getUrl(), hash(expected, downloader),
								hash(actual.getFile()));
					} catch (IOException e) {
						throw new AssertionError(e);
					}
